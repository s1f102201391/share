...
for node in graph:
    if node != 'u':
        ...
        distances_from_u[node] = (path, cost)

# Find the least cost path from node 'u' to 'e'
path_to_e, cost_to_e = find_least_cost_path(graph, 'u', 'e')

# Display the result
print("Nodes sorted by least distance from 'u':")
for node, (path, distance) in distances_from_u.items():
    print(f"Node {node}: {distance}, Path: {path}")

print(f"\nThe least cost path from node 'u' to node 'e' is: {path_to_e}")
print(f"The total cost of the path is: {cost_to_e}")
